...
export async function GET(
  ...
  try {
    ...
    const comments = await prisma.comment.findMany({
      where: { tutorialId: id, parentId: null },
      include: {
...
export async function POST(
  ...
  try {
    ...
    // If replying, verify parent comment exists and belongs to same tutorial
    if (parentId) {
      const parent = await prisma.comment.findUnique({ where: { id: parentId } });
      if (!parent || parent.tutorialId !== id) {
    ...
    // Log the comment with IP address
